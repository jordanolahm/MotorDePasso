...
load ledBranco_15.mat
white_led_15 = tensao; 

%Com lente
Ipd_l15 = (white_led_15 - background) / R; 
 
%recorte lateral do plot
Ipd_l15 = Ipd_l15(3:end-2);
phi_r = phi_r(3:end-2);

% Função o qual deve ser estimado o parâmetro nL
fun = @(x) x(2)*(x(1) + 1).*cos(phi_r).^x(1) - Ipd_l15;
%% Estimador NL-LS
%help lsqnonlin
...
mse = sum((Ipd_l15 - Ipd_hat).^2)/length(Ipd_l15);
rmse = sqrt(mse)

save param_15 Ipd_hat Ipd_l15;


figure;
plot( phi_r,Ipd_l15,'r', 'LineWidth', 1); hold on;
