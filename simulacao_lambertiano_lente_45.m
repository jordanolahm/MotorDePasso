...
load ledBranco_45.mat
white_led_45 = tensao; 

%Com lente 
Ipd_l45 = (white_led_45 - background) / R; 

%recorte lateral do plot
Ipd_l45 = Ipd_l45(3:end-2);
phi_r = phi_r(3:end-2);

% Função o qual deve ser estimado o parâmetro nL
fun = @(x) x(2)*(x(1) + 1).*cos(phi_r).^x(1) - Ipd_l45;
%% Estimador NL-LS
%help lsqnonlin
...
mse = sum((Ipd_l45 - Ipd_hat).^2)/length(Ipd_l45);
rmse = sqrt(mse)

save param_45 Ipd_hat Ipd_l45;

figure;
plot( phi_r,Ipd_l45,'r', 'LineWidth', 1); hold on;
